...
/// Type alias for the Ollama models cache
pub type ModelsCache = TtlCache<Vec<ModelDescriptor>>;
...
/// completions are routed to the cloud.
    ...
    S: MemoryStorage + 'static,
{
    ...
    client: C,
    /// The single source of truth for the instance's host, credentials and locality.
    ...
    ///
    /// This is shared across all [`OllamaProvider`] instances with the same endpoint.
    ...
    S: MemoryStorage + 'static,
{
    ...
    ///
    /// The provided [`ModelsCache`] should be shared across all instances of
    /// [`OllamaProvider`] with the same endpoint to maximize cache hits and minimize load on the
    /// instance.
    pub fn new(
        endpoint: OllamaEndpoint,
        runtime: OllamaModelRuntime<S>,
        ...
        Self::with_client(client, endpoint, runtime, models)
    }
}
    ...
    S: MemoryStorage + 'static,
{
    ...
    /// locality and credential facts stamped onto every model are taken from
    /// `endpoint`, never from `client`, so even a mismatched client cannot make
    /// a local-labelled model complete against the cloud — the client only
    ...
    async fn fetch_models(&self) -> ProviderResult<Vec<ModelDescriptor>> {
    ...
    S: MemoryStorage + 'static,
{
    ...
    async fn resolve(&self, reference: &ModelReference) -> ProviderResult<Arc<dyn Model>> {
        ...
        Ok(Arc::new(
            OllamaModel::new(&self.endpoint, &self.runtime, descriptor).await?,
        ))
    }
...
mod tests {
    ...
    fn should_stamp_cloud_descriptor_from_a_cloud_endpoint() {
        ...
        let provider = provider_with(
            OllamaEndpoint::cloud(SecretString::from("sk-ollama")),
            vec![],
        );
    ...
    async fn should_serve_repeat_lookups_from_the_cache() {
        ...
        provider.list_models().await.expect("first listing");
        provider.list_models().await.expect("second listing");
    ...
    async fn should_resolve_an_installed_model() {
        ...
        let resolved = provider.resolve(&reference).await;
