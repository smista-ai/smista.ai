//! A [`ureq`]-backed [`Client`], a blocking HTTP backend for the router API.
//!
//! [`UreqClient`] is a concrete [`Client`](crate::Client) implementation built on
//! [`ureq`], a small synchronous HTTP client with no async runtime of its own. It
//! is gated behind the crate's `ureq` feature and exists for frontends that want a
//! minimal dependency footprint (no `tokio`, no `reqwest`) while still programming
//! against the same [`Client`](crate::Client) trait as [`ReqwestClient`].
//!
//! # Blocking under an async trait
//!
//! The [`Client`](crate::Client) trait is `async`, but [`ureq`] is synchronous, so
//! every method here performs blocking I/O and returns an immediately-ready
//! future: there is no `.await` inside, the network round trip happens before the
//! future resolves. This is intentional and safe to drive on **any** executor,
//! even a trivial one such as [`futures::executor::block_on`] — there is no hidden
//! reactor to register with. The trade-off is that a method call occupies its
//! thread for the whole request, so when calling it from a work-stealing async
//! runtime (for example Tokio) you should offload it with the runtime's blocking
//! facility (such as `tokio::task::spawn_blocking`) rather than blocking a worker
//! thread. The streaming methods return a [`Stream`](futures::Stream) whose every
//! poll likewise reads from the socket synchronously.
//!
//! # Held state
//!
...
//! at the root. The agent is configured to surface a non-success HTTP status as a
//! normal response (not an error), so the router's structured [`ApiError`] body is
//! decoded and paired with the status; transport and decode failures map to
//! [`RouterClientError`]. The streaming methods negotiate `text/event-stream` and
//! parse the Server-Sent Events frames into [`TurnEvent`]s, ending after the
//! terminal `turn_end`.
//!
...
/// A [`ureq`]-backed, blocking [`Client`](crate::Client) implementation.
///
...
/// round trip.
///
/// Every method blocks the calling thread for the duration of the request: the
/// [`Client`](crate::Client) methods are `async` but contain no `.await`, so they
/// resolve on any executor without a `tokio` reactor. When driving them from a
/// work-stealing runtime such as Tokio, offload each call with the runtime's
/// blocking facility (for example `tokio::task::spawn_blocking`) rather than
/// blocking a worker thread.
///
/// # Examples
///
/// ```no_run
/// use smista_router_client::{Client, RouterClientConfig, UreqClient};
///
/// # fn run() -> smista_router_client::Result<()> {
/// let client = UreqClient::new(RouterClientConfig::default())?;
/// // The methods are `async` but block internally, so a trivial executor drives
/// // them; offload to a blocking task when running under a work-stealing runtime.
/// futures::executor::block_on(async {
///     client.bootstrap().await?;
///     client.sign_in().await?;
///     let sessions = client.list_sessions(None, None).await?;
///     # let _ = sessions;
///     smista_router_client::Result::Ok(())
/// })?;
/// # Ok(())
/// # }
/// ```
...
impl TryFrom<RouterClientConfig> for UreqClient {
    ...
    fn try_from(config: RouterClientConfig) -> Result<Self> {
        // `ureq` reports a non-success HTTP status as an `Err` by default; turn
        // that off so a `4xx`/`5xx` comes back as a readable response and the
        // router's structured `ApiError` body can be decoded from it.
        let agent_config = Agent::config_builder()
            .timeout_connect(Some(config.connect_timeout))
            .timeout_global(Some(config.request_timeout))
            .http_status_as_error(false)
            .build();
        let client = Agent::new_with_config(agent_config);
...
impl UreqClient {
    ...
    fn authorize<Any>(&self, builder: RequestBuilder<Any>) -> Result<RequestBuilder<Any>> {
    ...
    fn provider_headers<Any>(&self, builder: RequestBuilder<Any>) -> RequestBuilder<Any> {
        self.credentials
            .headers_map()
            .into_iter()
...
fn decode<T>(mut response: Response<Body>) -> Result<T>
where
...
/// Opens `response` as a Server-Sent Events stream of [`TurnEvent`]s.
///
/// A non-success status is mapped to an [`ApiError`] before the stream opens;
/// once open, each event is parsed lazily and a mid-stream failure surfaces as an
/// [`Err`] item.
fn open_stream(response: Response<Body>) -> Result<BoxStream<'static, Result<TurnEvent>>> {
    if response.status().is_success() {
...
/// Logs and wraps a [`ureq`] transport failure as a [`RouterClientError`].
///
...
fn transport_error(error: ureq::Error) -> RouterClientError {
...
fn decode_api_error(mut response: Response<Body>) -> RouterClientError {
...
/// A blocking iterator over the [`TurnEvent`]s of an open SSE response body.
///
/// Each call to [`next`](Iterator::next) reads from the socket synchronously,
/// draining every complete record into the queue before yielding the next event.
struct SseEvents {
    /// The blocking reader over the response body.
        ...
        loop {
            ...
            match self.reader.read(&mut chunk) {
                Ok(0) => {
                    ...
                    parse_sse_record(&trailing, &mut self.queue);
                    self.finished = true;
                }
                Ok(read) => {
                    self.buffer.extend_from_slice(&chunk[..read]);
                    drain_sse_records(&mut self.buffer, &mut self.queue);
                }
                Err(error) => {
                    // A transport failure ends the turn after it is surfaced.
                    self.queue.push_back(Err(read_error(error)));
                    self.finished = true;
                }
            }
        }
...
/// surfaces as an [`Err`] item and ends the stream. Every poll reads from the
/// socket synchronously, so the stream blocks the polling thread.
fn sse_events(response: Response<Body>) -> BoxStream<'static, Result<TurnEvent>> {
...
impl Client for UreqClient {
    ...
    async fn sign_in(&self) -> Result<SignInResponse> {
        ...
        if let Some(api_key) = self
            .api_key
            .read()
            .expect("the API key lock is not poisoned")
            .as_ref()
        {
            builder = builder.header(ApiKey::header_name(), api_key.expose());
        }
        let raw = builder.send_empty().map_err(transport_error)?;
        let response: SignInResponse = decode(raw)?;
        // Hold the minted session token for subsequent authenticated calls.
        *self
            .session_token
            .write()
            .expect("the session token lock is not poisoned") = Some(
    ...
    async fn sign_out(&self) -> Result<SignOutResponse> {
        ...
        let builder = self.authorize(self.client.post(url.as_str()))?;
        let raw = builder.send_empty().map_err(transport_error)?;
        let response: SignOutResponse = decode(raw)?;
        // The token is revoked server-side; drop the held copy too.
        *self
            .session_token
            .write()
            .expect("the session token lock is not poisoned") = None;
        Ok(response)
    }

    async fn me(&self) -> Result<MeResponse> {
        let url = self.url("/api/v1/auth/me")?;
        let builder = self.authorize(self.client.get(url.as_str()))?;
        decode(builder.call().map_err(transport_error)?)
    }

    async fn create_session(&self, req: CreateSessionRequest) -> Result<CreateSessionResponse> {
        let url = self.url("/api/v1/sessions")?;
        let builder = self.authorize(self.client.post(url.as_str()))?;
        decode(builder.send_json(&req).map_err(transport_error)?)
    }
    ...
    async fn update_session(
        ...
        let url = self.url(&format!("/api/v1/sessions/{id}"))?;
        let builder = self.authorize(self.client.put(url.as_str()))?;
        decode(builder.send_json(&req).map_err(transport_error)?)
    }

    async fn delete_session(&self, id: Uuid) -> Result<DeleteSessionResponse> {
        let url = self.url(&format!("/api/v1/sessions/{id}"))?;
        let builder = self.authorize(self.client.delete(url.as_str()))?;
        decode(builder.call().map_err(transport_error)?)
    }
    ...
    async fn stream_execute(
        ...
        let raw = builder.send_json(&req).map_err(transport_error)?;
        open_stream(raw)
    }
...
mod tests {
    ...
    fn client_for(router: &MockRouter) -> UreqClient {
        UreqClient::new(RouterClientConfig::new(router.base_url())).expect("the client builds")
    }

    /// Builds a client already holding a session token, by signing in first.
    async fn signed_in_client(router: &MockRouter) -> UreqClient {
    ...
    fn should_get_base_url() {
        let client = UreqClient::new(RouterClientConfig::default()).expect("the client builds");
