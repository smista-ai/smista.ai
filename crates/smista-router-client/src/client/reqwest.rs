//! A [`reqwest`]-backed [`Client`], the default router backend.
//!
//! [`ReqwestClient`] is the concrete, best-supported [`Client`](crate::Client)
//! implementation: an `async` HTTP client over [`reqwest`], gated behind the
//! crate's `reqwest` feature. It is the backend `smista-cli` and other Rust
//! frontends use to talk to a running `smista-router`.
//!
//! # Held state
//!
...
/// The default, [`reqwest`]-backed [`Client`](crate::Client) implementation.
///
...
/// router. It owns its credentials, so the [`Client`](crate::Client) methods
/// take `&self` with no per-call credential; a [`Clone`] shares the same auth
/// state; and an authenticated call made before a token is held fails with
...
///
/// ```no_run
/// use smista_router_client::{Client, ReqwestClient, RouterClientConfig};
///
...
impl TryFrom<RouterClientConfig> for ReqwestClient {
...
impl ReqwestClient {
    ...
    pub fn with_api_key(mut self, api_key: ApiKey) -> Self {
        // Replace the lock rather than write through it: at construction time no
        // clone shares this `Arc` yet, and replacing avoids `blocking_write`,
        // which would panic if called from within an async runtime.
    ...
    pub fn with_session_token(mut self, session_token: SessionToken) -> Self {
        // See `with_api_key` for why the lock is replaced rather than written.
        self.session_token = Arc::new(RwLock::new(Some(session_token)));
        self
    }
    ...
    /// Sends `builder` and decodes a success body or maps an error response.
    async fn send<T>(&self, builder: RequestBuilder) -> Result<T>
    where
    ...
    /// Opens `builder` as a Server-Sent Events stream of [`TurnEvent`]s.
    ///
    /// The request negotiates `text/event-stream`. A non-success status is mapped
    /// to an [`ApiError`] before the stream opens; once open, each event is parsed
    /// lazily and a mid-stream failure surfaces as an [`Err`] item.
...
/// Logs and wraps a [`reqwest`] transport failure as a [`RouterClientError`].
///
...
fn transport_error(error: reqwest::Error) -> RouterClientError {
...
async fn decode_api_error(response: Response) -> RouterClientError {
...
struct SseState {
    /// The raw response byte stream, chunked by the network.
...
/// surfaces as an [`Err`] item and ends the stream.
fn sse_events(response: Response) -> BoxStream<'static, Result<TurnEvent>> {
    let bytes = response
        .bytes_stream()
        ...
        loop {
            ...
            match state.bytes.next().await {
                ...
                Some(Err(error)) => {
                    ...
                    state.queue.push_back(Err(transport_error(error)));
                    state.finished = true;
                }
                None => {
...
impl Client for ReqwestClient {
    ...
    async fn bootstrap(&self) -> Result<BootstrapResponse> {
        ...
        let response: BootstrapResponse = self.send(self.client.post(url)).await?;
        // Bootstrap mints the long-lived API key exactly once: hold it so a later
        // `sign_in` can present it.
        *self.api_key.write().await = Some(
    ...
    async fn sign_in(&self) -> Result<SignInResponse> {
        ...
        if let Some(api_key) = self.api_key.read().await.as_ref() {
            builder = builder.header(ApiKey::header_name(), api_key.expose());
        }
        let response: SignInResponse = self.send(builder).await?;
        // Hold the minted session token for subsequent authenticated calls.
        *self.session_token.write().await = Some(
    ...
    async fn sign_out(&self) -> Result<SignOutResponse> {
        ...
        let builder = self.authorize(self.client.post(url)).await?;
        let response: SignOutResponse = self.send(builder).await?;
    ...
    async fn create_session(&self, req: CreateSessionRequest) -> Result<CreateSessionResponse> {
        ...
        let builder = self.authorize(self.client.post(url)).await?.json(&req);
        self.send(builder).await
    }
    ...
    async fn list_sessions(
        ...
        let url = self.url("/api/v1/sessions")?;
        // Omitted filters fall back to the router's defaults.
        ...
        let builder = self.authorize(self.client.get(url)).await?.query(&query);
        self.send(builder).await
    }
    ...
    async fn update_session(
        ...
        let url = self.url(&format!("/api/v1/sessions/{id}"))?;
        let builder = self.authorize(self.client.put(url)).await?.json(&req);
        self.send(builder).await
    }

    async fn delete_session(&self, id: Uuid) -> Result<DeleteSessionResponse> {
        let url = self.url(&format!("/api/v1/sessions/{id}"))?;
        let builder = self.authorize(self.client.delete(url)).await?;
        self.send(builder).await
    }
    ...
    async fn execute(&self, id: Uuid, req: ExecuteRequest) -> Result<TurnResponse> {
        ...
        let builder = self.authorize(self.client.post(url)).await?;
        let builder = self.provider_headers(builder).json(&req);
        self.send(builder).await
    }
    ...
    async fn stream_execute(
        ...
        let builder = self.authorize(self.client.post(url)).await?;
        let builder = self.provider_headers(builder).json(&req);
        self.open_stream(builder).await
    }
    ...
    async fn get_session_traces(
        ...
        let url = self.url(&format!("/api/v1/sessions/{id}/traces"))?;
...
mod tests {
    ...
    fn client_for(router: &MockRouter) -> ReqwestClient {
        ReqwestClient::new(RouterClientConfig::new(router.base_url())).expect("the client builds")
    }
    ...
    async fn should_seed_credentials_through_the_builders() {
        let router = MockRouter::start().await;
        // The builders must not panic when called from within an async runtime.
        let client = client_for(&router)
