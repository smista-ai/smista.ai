...
mod tests {
    ...
    use crate::app::tui::state::prompt::command::{
        COMMAND_CHAT, COMMAND_CLEAR, COMMAND_HELP, COMMAND_LOGS, COMMAND_MODEL, COMMAND_PLAN,
        COMMAND_PREVIEW, COMMAND_PROVIDERS, COMMAND_QUIT, COMMAND_RESUME, COMMAND_SKILLS,
        COMMAND_STATUS, COMMAND_TRACE, COMMAND_USAGE,
    };
    ...
    fn should_parse_help() {
        ...
        assert_eq!(state.command(), Some(&Command::Help));
        assert_eq!(state.command_args(), Some(""));
        assert_eq!(state.input(), "/help");
        assert_eq!(state.command().map(Command::input_name), Some(COMMAND_HELP));
    }
    ...
    #[test]
    fn should_parse_trace() {
        let mut state = PromptState::default();

        state.push_str("/trace");

        assert_eq!(state.command(), Some(&Command::Trace));
        assert_eq!(state.command_args(), Some(""));
        assert_eq!(state.input(), "/trace");
        assert_eq!(
            state.command().map(Command::input_name),
            Some(COMMAND_TRACE)
        );
